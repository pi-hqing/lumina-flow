...
- [index.html](index.html)、[spotlight.css](spotlight.css)、[spotlight.js](spotlight.js) 只作视觉与动效参考，不作为交互规则或边界定义的来源。
...
- **[spotlight.css](spotlight.css)**：Spotlight Command Center 样式（搜索栏、Source Tabs、结果列表、预览面板、AI Summary Drawer）。
- **[spotlight.js](spotlight.js)**：Spotlight Command Center 交互逻辑（搜索过滤、键盘导航、预览展开、AI 摘要、Mock 数据）。
